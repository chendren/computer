---
description: Speak text aloud using Amazon Nova Sonic TTS
...
Convert text to speech using Amazon Nova 2 Sonic via Bedrock and play through speakers.

## Arguments

- `$ARGUMENTS` contains the text to speak, optionally with flags:
  - `--voice <name>` — choose a voice (default: tiffany). Options: tiffany, matthew, amy, ambre, florian, beatrice, lorenzo, greta, lennart, lupe, carlos
...
1. **Parse arguments**: Extract text and flags from `$ARGUMENTS`.
   - Default voice: tiffany
   - Default output: `/tmp/nova-speak-output.wav`
...
- `/computer:nova-speak Hello Captain, all systems nominal`
- `/computer:nova-speak --voice matthew Warp core online, shields at maximum`
- `/computer:nova-speak --save ~/Desktop/alert.wav Red alert, battle stations`
